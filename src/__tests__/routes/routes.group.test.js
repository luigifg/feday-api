...
const routes = require("../../routes/group")
...
describe("Testes do banco de dados", () => {
  test("GET", async () => {
    ...
    const response = await agent.get("/group")
  ...
  test("POST API.INSERT", async () => {
    ...
    const response = await agent.post("/group").send({
      name: "Teste",
  ...
  test("POST API.SEARCH", async () => {
    ...
    const response = await agent.post("/group/search").send(params)
  ...
  test("GET API.GETBYID", async () => {
    ...
    const response = await agent.get(`/group/${id}`)
    expect(response.status).toBe(200)
  })
  ...
  test("PATCH API.UPDATE", async () => {
    ...
    const response = await agent.patch(`/group/${id}`).send(params)
  ...
  test("DELETE API.REMOVE", async () => {
    ...
    const response = await agent.delete(`/group/${id}`)
