...
describe("Testes do banco de dados", () => {
  test("GET", async () => {
    ...
    const response = await agent.get("/acl")
  ...
  test("POST API.INSERT", async () => {
    ...
    const response = await agent.post("/acl").send({
        idGroup: 1,
        idScreen: 1,
    })
  ...
  test("POST API.SEARCH", async () => {
    ...
    const params = [{ column: "idGroup", signal: "=", value: 1 }]

    const response = await agent.post("/acl/search").send(params)
  ...
  test("POST API.SEARCH INCORRETO", async () => {
    ...
    const params = [{ column: "idGroup", signal: "=", value: "teste" }]
  ...
  test("GET API.GETBYID", async () => {
    ...
    const response = await agent.get(`/acl/${id}`)
    expect(response.status).toBe(200)
  })
  ...
  test("PATCH API.UPDATE", async () => {
    ...
    const response = await agent.patch(`/acl/${id}`).send(params)
  ...
  test("DELETE API.REMOVE", async () => {
    ...
    const response = await agent.delete(`/acl/${id}`)
